...
| `When I input "cat" in address bar` | `tapOn, inputText, pressKey, waitUntil` | Conservative semantic conversion. |
| `And I press the "Enter" key` | `pressKey` | Conservative semantic conversion. |
| `Then the tab should jump to the search results page related to "cat"` | `assert/assertVisible/assertWithAI` | Conservative semantic conversion. |
