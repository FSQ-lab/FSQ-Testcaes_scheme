...
| `And I navigate to "https://www.bbc.com"` | `executeMethod: native_navigate` | Preserves Windows pywinauto MCP `native_navigate` source operation. |
| `And I open a new tab` | `tapOn` | Converted from matched step implementation. |
| `And I navigate to "https://www.apple.com"` | `executeMethod: native_navigate` | Preserves Windows pywinauto MCP `native_navigate` source operation. |
| `And I close and restart Edge` | `killApp, launchApp` | Preserves source restart semantics with explicit lifecycle commands. |
